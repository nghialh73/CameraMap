...
public class MainUI {
    ...
    public MainUI(HomeActivity main_activity) {
        if( Debug.LOG )
            Log.d(TAG, "MainUI");
...
//        view.animate().rotationBy(rotate_by).setDuration(100).setInterpolator(new AccelerateDecelerateInterpolator()).start();
    ...
    public void layoutUI() {
        ...
        String ui_placement = sharedPreferences.getString(Keys.getUIPlacementPreferenceKey(), "ui_right");
...
//            layoutParams.addRule(align_top, R.id.gallery);
//            layoutParams.addRule(align_bottom, R.id.gallery);
//            view.setLayoutParams(layoutParams);
//            setViewRotation(view, ui_rotation);
...
//            view.setLayoutParams(layoutParams);
    ...
    public void setTakePhotoIcon() {
        ...
        if( main_activity.getPreview() != null ) {
            ...
            if( main_activity.getPreview().isVideo() ) {
                ...
                resource = main_activity.getPreview().isVideoRecording() ? R.drawable.take_video_recording : R.drawable.video_selector;
                ...
                resource = R.drawable.photo_selector;
    ...
    public void setImmersiveMode(final boolean immersive_mode) {
        ...
        main_activity.runOnUiThread(new Runnable() {
            public void run() {
                ...
                if( main_activity.getPreview().supportsZoom() && sharedPreferences.getBoolean(Keys.getShowZoomControlsPreferenceKey(), false) ) {
                    zoomControls.setVisibility(visibility);
                }
                if( main_activity.getPreview().supportsZoom() && sharedPreferences.getBoolean(Keys.getShowZoomSliderControlsPreferenceKey(), true) ) {
                    zoomSeekBar.setVisibility(visibility);
                }
                String pref_immersive_mode = sharedPreferences.getString(Keys.getImmersiveModePreferenceKey(), "immersive_mode_low_profile");
    ...
    public void toggleExposureUI() {
        ...
        if( is_open ) {
            ...
            SharedPreferences sharedPreferences = PreferenceManager.getDefaultSharedPreferences(main_activity);
            String value = sharedPreferences.getString(Keys.getISOPreferenceKey(), main_activity.getPreview().getCameraController().getDefaultISO());
    ...
    public void closePopup() {
        ...
        if( popupIsOpen() ) {
             ...
             *     HomeActivity.updateForSettings(), but doing so makes the popup close when checking photo or video resolutions!
