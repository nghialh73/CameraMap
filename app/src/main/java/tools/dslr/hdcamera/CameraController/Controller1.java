...
public class Controller1 extends Controller {
    ...
    public Controller1(int cameraId) throws ControllerException {
        super(cameraId);
        if( Debug.LOG )
            Log.d(TAG, "create new Controller1: " + cameraId);
        try {
    ...
    private static class CameraErrorCallback implements Camera.ErrorCallback {
        ...
        public void onError(int error, Camera camera) {
            // n.b., as this is potentially serious error, we always log even if Debug.LOG is false
    ...
    public CameraFeatures getCameraFeatures() {
        ...
        //camera_features.picture_sizes.add(new Controller.Size(1920, 1080)); // test
        for(Camera.Size camera_size : camera_picture_sizes) {
            camera_features.picture_sizes.add(new Controller.Size(camera_size.width, camera_size.height));
        }
        ...
        //camera_features.video_sizes.add(new Controller.Size(1920, 1080)); // test
        for(Camera.Size camera_size : camera_video_sizes) {
            camera_features.video_sizes.add(new Controller.Size(camera_size.width, camera_size.height));
        }
        ...
        for(Camera.Size camera_size : camera_preview_sizes) {
            camera_features.preview_sizes.add(new Controller.Size(camera_size.width, camera_size.height));
        }
    ...
    @Override
    public Controller.Size getPictureSize() {
        Camera.Parameters parameters = this.getParameters();
        Camera.Size camera_size = parameters.getPictureSize();
        return new Controller.Size(camera_size.width, camera_size.height);
    }
    ...
    @Override
    public Controller.Size getPreviewSize() {
        Camera.Parameters parameters = this.getParameters();
    ...
    public List<Area> getFocusAreas() {
        ...
        for(Camera.Area camera_area : camera_areas) {
            areas.add(new Controller.Area(camera_area.rect, camera_area.weight));
        }
        return areas;
    }
    ...
    @Override
    public
    void reconnect() throws ControllerException {
        if( Debug.LOG )
            Log.d(TAG, "reconnect");
        try {
            camera.reconnect();
    ...
    public void setFaceDetectionListener(final Controller.FaceDetectionListener listener) {
    ...
    public void autoFocus(final Controller.AutoFocusCallback cb) {
        if( Debug.LOG )
            Log.d(TAG, "autoFocus");
    ...
    private void takePictureNow(final Controller.PictureCallback picture, final ErrorCallback error) {
        if( Debug.LOG )
    ...
    public void takePicture(final Controller.PictureCallback picture, final ErrorCallback error) {
        if( Debug.LOG )
        ...
        if( frontscreen_flash ) {
            ...
            // take picture after a delay, to allow autoexposure and autofocus to update (unlike Controller2, we can't tell when this happens, so we just wait for a fixed delay)
            Handler handler = new Handler();
            handler.postDelayed(new Runnable(){
                @Override
                public void run(){
