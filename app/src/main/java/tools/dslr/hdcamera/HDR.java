...
public class HDR {
	...
	protected void onDestroy() {
		if( Debug.LOG )
			Log.d(TAG, "onDestroy");
		if( rs != null ) {
			// need to destroy con1, otherwise this isn't necessarily garbage collected - we had tests failing with out of memory
			...
			rs.destroy(); // on Android M onwards this is a NOP - instead we call RenderScript.releaseAllContexts(); in HomeActivity.onDestroy()
		}
	}
	...
	private ResponseFunction createFunctionFromBitmaps(int id, Bitmap in_bitmap, Bitmap out_bitmap) {
		...
		ResponseFunction function = new ResponseFunction(con, id, x_samples, y_samples, weights);
		return function;
	}
	 ...
	 *  by around 1799KB! We don't care about pre-Android 4.4 (HDR requires Controller2 which requires
	 *  Android 5.0 anyway; even if we later added support for Controller1, we can simply say HDR requires
	 *  Android 5.0).
	 */
	...
	private void processHDRCore(List<Bitmap> bitmaps) {
		...
		if( use_renderscript ) {
			...
			if( rs == null ) {
				this.rs = RenderScript.create(con);
				if( Debug.LOG )
